...
public class ControllerHelper {
	...
	/**
	 * Add common header variables to modelRoot. Including locale, rootURL,
	 * currentVersion and useMinified
	 * 
	 * @param request
	 * @param locale
	 * @param appConfig
	 * @param modelRoot
	 */
	public static void setPageHeaderVariables(HttpServletRequest request, String resourceName, String[] resourceParams,
		...
		for (String currSupportedLang : appConfig.getSupportedLanguagesList()) {
			if (!currSupportedLang.equalsIgnoreCase(currLanguage)) {
				otherLanguagePath = I18nUrlBuilder.generateI18nResourcePath(currSupportedLang,
						OccurrencePortalConfig.I18N_TRANSLATION_HANDLER.getTranslationFormat(resourceName),
						resourceParams);
	 ...
	 * @param modelRoot
	 */
	public static void setResourceVariables(HttpServletRequest request, String resourceName, String resourceParam,
			OccurrencePortalConfig appConfig, HashMap<String, Object> modelRoot) {
		String[] resourceParams = new String[1];
		resourceParams[0] = resourceParam;
		setPageHeaderVariables(request, resourceName, resourceParams, appConfig, modelRoot);
	}
}
