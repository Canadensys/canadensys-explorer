...
EXPLORER.stats = (function() {
  ...
  function transformAltitudeData(json){
    ...
    formattedJson.push([EXPLORER.i18n.getLanguageResource('view.stats.chart.altitude.below') +" 0",0]);
    $.each(json, function() {
      if(EXPLORER.utils.isInteger(this[0])){
        if(this[0] < 0){
          formattedJson[0][1] = formattedJson[0][1] + this[1];
        }
        else if(this[0] >= MAX_ALTITUDE){
          aboveMaxMetersCount = aboveMaxMetersCount + this[1];
        }
        else{
          formattedJson.push(this);
        }
      }
    });
    formattedJson.push([EXPLORER.i18n.getLanguageResource('view.stats.chart.altitude.above')+" 2000",aboveMaxMetersCount]);
